...
#define DF_DIALOGACTIVE 0x4000 // InteractOS
...
typedef struct
{
    ...
    LPCWSTR    className;
    LPCWSTR    windowName;
    BOOL       windowNameFree; // InteractOS
    LPCVOID    data;
} DLG_CONTROL_INFO;
...
//// InteractOS see 43396, Fixes setting focus on Open and Close dialogs to the FileName edit control in OpenOffice.
//// This now breaks test_SaveRestoreFocus.
            //DEFDLG_SaveFocus( hwnd );
////
        }
//// InteractOS Rev 30613 & 30644
...
/*
 * @implemented Modified for InteractOS. Do not Port Sync!!!
 */
BOOL
WINAPI
EndDialog(
...
{
	HWND hwndCtrl;
	if ( hDlg == HWND_TOPMOST || hDlg == HWND_BROADCAST ) return 0; // InteractOS
...
SetDlgItemTextA(
  ...
  LPCSTR lpString)
{
  HWND hwndCtrl = GetDlgItem( hDlg, nIDDlgItem ); // InteractOS Themes
